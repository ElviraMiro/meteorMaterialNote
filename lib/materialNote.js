    ...
    var tplButton = function(label, options) {
      ...
      if (!dropdown) {
        ...
        var list = [
            '<div class="btn-group',
            (className ? " " + className : '') + '">',
            '<div class="waves-effect waves-light btn dropdown ' + (btnClassName ? btnClassName : '') + '"',
            (title ? ' title="' + title + '"' : ''),
            (event ? ' data-event="' + event + '"' : ''),
            (value ? ' data-value=\'' + value + '\'' : ''),
            (hide ? ' data-hide=\'' + hide + '\'' : ''),
            '><i class="material-icons left">arrow_drop_down</i>' + label + '</div>',
            dropdown,
            '</div>'
        ].join('');
      ...
      $container.find('.note-color-palette').each(function() {
        ...
        for (var row = 0, lenRow = colorInfo.length; row < lenRow; row++) {
          ...
          for (var col = 0, lenCol = colors.length; col < lenCol; col++) {
            ...
            buttons.push(['<div class="note-color-btn" style="background-color:', color,
                   ';" data-event="', eventName,
                   '" data-value="', color,
                   '" data-description="', title,
                   '" data-toggle="button" tabindex="-1"></div>'].join(''));
          }
          paletteContents.push('<div class="note-color-row">' + buttons.join('') + '</div>');
        }
        $palette.html(paletteContents.join(''));

        $palette.find('.note-color-btn').mouseenter(function() {
    ...
    materialnote: function() {
      ...
      $(this).each(function(index, editor) {
        ...
        if ($(editor).hasClass('note-air-editor')) {
          ...
          toolbar = $(editor).find('.popover-content div.dropdown');
          isAir = true;
        } else {
          editor = $(editor).next('.note-editor');
          tabContainer = editor.find('ul.tabs');
          tabs = editor.find('li.tab a');
          toolbar = $(editor).find('.note-toolbar div.dropdown');
        }
        var go = true;
        ...
        function handleDropdowns(select, bar) {
          ...
          $(select).click(function(event) {
            ...
            bar.find('ul.dropdown-menu').slideUp(2000);
        ...
        $(window).click(function(event) {
          if (go) editor.find('ul.dropdown-menu').slideUp(2000);
          go = true;
          event.stopPropagation();
        });
